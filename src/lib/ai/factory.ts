...
function getCapabilityRegistry(): Record<Capability, ProviderCapability[]> {
  ...
  const capabilityMap: Record<Capability, (p: AIProvider) => boolean> = {
...
export async function executeWithFallback<T>(
  ...
  const available = registry[capability]
  ...
  for (const { provider } of available) {
    try {
      ...
      console.warn(
        `Provider ${provider.id} failed for ${capability}:`,
        error
      )
    }
  }
  ...
  throw new Error(
    `All providers failed for ${capability}: ${errors
      .map((e) => e.message)
      .join(", ")}`
  )
}
